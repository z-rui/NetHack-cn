...
boolean quietly;
{
    ...
    if (vis && !quietly)
        pline("%s把%s从%s道路上移开了!", Monnam(magr), mon_nam(mdef),
 ...
 post_stone:
    ...
    case AD_SEDU:
        ...
        if (obj) {
            ...
            if (vis && canseemon(mdef)) {
                Strcpy(buf, Monnam(magr));
                pline("%s 从%s偷走了%s!", buf, mdefnambuf, onambuf);
            }
