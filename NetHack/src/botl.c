...
do_statusline2()
{
    ...
    if (hp < 0)
        hp = 0;
    Sprintf(hlth, "生命:%d(%d) 魔力:%d(%d) AC:%-2d",
    ...
    if (Upolyd)
        ...
        Sprintf(expr, "经验:%u/%-1ld", u.ulevel, u.uexp);
    else
        Sprintf(expr, "等级:%u", u.ulevel);
    xln = strlen(expr);

    /* time/move counter */
    if (flags.time)
        Sprintf(tmmv, "回合:%ld", moves);
    else
        tmmv[0] = '\0';
    tln = strlen(tmmv);
...
STATIC_DCL struct istat_s initblstats[MAXBLSTATS] = {
    ...
    INIT_BLSTAT("score", " 得分:%s", ANY_LONG, 20, BL_SCORE),
    ...
    INIT_BLSTAT("experience-level", " 经验:%s", ANY_LONG, 10, BL_XP),
    INIT_BLSTAT("armor-class", " AC:%s", ANY_INT, 10, BL_AC),
    INIT_BLSTAT("HD", " HD:%s", ANY_INT, 10, BL_HD),
    INIT_BLSTAT("time", " 回合:%s", ANY_INT, 20, BL_TIME),
...
/* format arguments */
static const char threshold_value[] = "hilite_status 阈值 ",
                  is_out_of_range[] = " 超出范围";
        ...
        for (hl = blstats[0][fldidx].thresholds; hl; hl = hl->next) {
            ...
            case BL_TH_TEXTMATCH: /* ANY_STR */
                ...
                if (fldidx == BL_TITLE)
                    ...
                    txtstr += (strlen(plname) + sizeof "  " - sizeof "");
...
choose_value:
    if (behavior == BL_TH_VAL_PERCENTAGE
        ...
        Sprintf(colorqry, "选择当%s%s%s时的颜色:",
                initblstats[fld].fldname,
                (lt_gt_eq == LT_VALUE) ? "小于 "
                  : (lt_gt_eq == GT_VALUE) ? "大于 "
                    : (lt_gt_eq == LE_VALUE) ? "大于等于"
                      : (lt_gt_eq == GE_VALUE) ? "小于等于"
                        : "等于",
                numstart);
        Sprintf(attrqry, "选择当%s%s%s时的属性:",
                initblstats[fld].fldname,
                (lt_gt_eq == LT_VALUE) ? "小于 "
                  : (lt_gt_eq == GT_VALUE) ? "大于 "
                    : (lt_gt_eq == LE_VALUE) ? "大于等于"
        ...
        Sprintf(colorqry, "选择当%s%s时的颜色:",
                initblstats[fld].fldname,
                (lt_gt_eq == EQ_VALUE) ? "变化"
                  : (lt_gt_eq == LT_VALUE) ? "减小"
                    : "增大");
        Sprintf(attrqry, "选择当%s%s时的属性:",
                initblstats[fld].fldname,
                (lt_gt_eq == EQ_VALUE) ? "变化"
                  : (lt_gt_eq == LT_VALUE) ? "减小"
                    : "增大");
        ...
        Sprintf(colorqry, "选择条件%s的颜色:",
                conditionbitmask2str(cond));
        Sprintf(attrqry, "选择条件%s的属性:",
                conditionbitmask2str(cond));
    } else if (behavior == BL_TH_TEXTMATCH) {
        char qry_buf[BUFSZ];

        Sprintf(qry_buf, "%s要匹配的%s文本值:",
                (fld == BL_CAP
                 || fld == BL_ALIGN
                 || fld == BL_HUNGER
                 || fld == BL_TITLE) ? "选择" : "输入",
