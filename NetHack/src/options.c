...
static struct Comp_Opt {
    ...
    { "name", "你的角色名字 (e.g., name:Merlin-W)", PL_NSIZ,
      DISP_IN_GAME },
    { "mouse_support", "游戏接收鼠标信息", 0, SET_IN_GAME },
...
#else
      "调色板 (在调色板里调整RGB颜色 (颜色-R-G-B)", 15, SET_IN_FILE
#endif
    },
#if defined(MAC)
    ...
    { "paranoid_confirmation", "extra prompting in certain situations", 28,
      SET_IN_GAME },
    { "petattr",  "高亮宠物的属性", 88, SET_IN_GAME },
    { "pettype", "首选的初始宠物类型", 4, DISP_IN_GAME },
    { "pickup_burden", "提示之前的拾取最大负重", 20,
      SET_IN_GAME },
    { "pickup_types", "自动拾取的物品类型",
      MAXOCLASSES, SET_IN_GAME },
    { "pile_limit", "阈值 \"这里有很多物品\"", 24,
      SET_IN_GAME },
    { "playmode", "一般模式, 无分数探索模式, 或调试模式", 8,
      DISP_IN_GAME },
    { "player_selection", "通过对话框和提示选择角色", 12,
      DISP_IN_GAME },
    { "race", "你的起始种族 (如, 人类, 精灵)", PL_CSIZ,
      DISP_IN_GAME },
    { "role", "你的起始职业 (如, 野蛮人, 女武神)", PL_CSIZ,
      DISP_IN_GAME },
    { "runmode", "`跑' 或 `旅行' 时的显示频率",
      sizeof "teleport", SET_IN_GAME },
    { "scores", "你想要看的分数列表部分", 32,
      SET_IN_GAME },
    { "scroll_amount", "到达 scroll_margin 时地图的滚动量",
      20, DISP_IN_GAME }, /*WC*/
    { "scroll_margin", "当离边缘很远滚动地图", 20,
      DISP_IN_GAME }, /*WC*/
    { "sortloot", "按照描述排序物品选择列表", 4,
      SET_IN_GAME },
#ifdef MSDOS
    ...
    { "symset", "从符号文件中加载一组显示符号", 70,
      SET_IN_GAME },
    { "roguesymset",
      "从符号文件加载一组 rogue 显示符号", 70,
      SET_IN_GAME },
#ifdef WIN32
    { "subkeyvalue", "覆盖键值", 7, SET_IN_FILE },
#endif
    { "suppress_alert", "抑制版本相关的警报", 8,
      SET_IN_GAME },
    /* term_cols,term_rows -> WC2_TERM_SIZE (6: room to format 1..32767) */
    { "term_cols", "列数", 6, SET_IN_FILE }, /*WC2*/
    { "term_rows", "行数", 6, SET_IN_FILE }, /*WC2*/
    { "tile_width", "块宽", 20, DISP_IN_GAME },   /*WC*/
    { "tile_height", "块高", 20, DISP_IN_GAME }, /*WC*/
    { "tile_file", "块文件的名字", 70, DISP_IN_GAME }, /*WC*/
    { "traps", "绘制陷阱使用的符号", MAXTCHARS + 1,
      SET_IN_FILE },
    { "vary_msgcount", "一次显示更多的旧信息", 20,
      DISP_IN_GAME }, /*WC*/
#ifdef MSDOS
...
#endif
#ifdef BACKWARD_COMPAT
    { "DECgraphics", "加载 DECGraphics 显示符号", 70, SET_IN_FILE },
    { "IBMgraphics", "加载 IBMGraphics 显示符号", 70, SET_IN_FILE },
#ifdef CURSES_GRAPHICS
    { "cursesgraphics", "加载 curses 显示符号", 70, SET_IN_FILE },
#endif
...
option_help()
{
    ...
    Sprintf(buf, "在%s 像这样设置选项: OPTIONS=<options>", configfile);
...
struct fruit *replace_fruit;
{
    ...
    if (user_specified) {
        ...
        if (found || numeric
            ...
            Strcpy(buf, pl_fruit);
            Strcpy(pl_fruit, "糖制的");
            nmcpy(pl_fruit + strlen("糖制的"), buf, PL_FSIZ - 9);
        }
        *altname = '\0';
