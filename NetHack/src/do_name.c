...
const char *goal;
{
    ...
    if (!iflags.terrainmode) {
        ...
        Sprintf(sbuf, "当你在正确的位置时输入%s.", kbuf);
        putstr(tmpwin, 0, sbuf);
        if (doing_what_is) {
            Sprintf(sbuf,
...
int gloc;
{
    ...
    Sprintf(tmpbuf, "选择%s的一个%s%s",
            gloc_filtertxt[iflags.getloc_filter],
            gloc_descr[gloc][1],
            iflags.getloc_travelmode ? "作为目的地" : "");
