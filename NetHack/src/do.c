...
polymorph_sink()
{
    ...
    if (levl[u.ux][u.uy].typ != ROOM)
        pline_The("水槽变形成了%s!", an(defsyms[sym].explanation));
    else
        pline_The("水槽消失了.");
    newsym(u.ux, u.uy);
}
...
register struct obj *obj;
{
    ...
    case RIN_SEARCHING:
        You("以为%s 在水槽里丢了, 原来它就在那!", yname(obj));
        goto giveback;
